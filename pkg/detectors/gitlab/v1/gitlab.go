...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for token := range uniqueMatches {
		...
		if verify {
			...
			isVerified, extraData, verificationErr := s.verifyGitlab(ctx, s.client, token)
...
func (s Scanner) verifyGitlab(ctx context.Context, client *http.Client, resMatch string) (bool, map[string]string, error) {
	...
	for _, baseURL := range s.Endpoints() {
		...
		default:
			return false, nil, fmt.Errorf("unexpected HTTP response status %d, body = %q", res.StatusCode, string(bodyBytes))
		}
	}
