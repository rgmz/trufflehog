...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		if verify {
			...
			go func() {
				defer wg.Done()
				user, err := verifyGitLabUser("gitlab.com", parsedKey)
				if err != nil && !errors.Is(err, errPermissionDenied) {
					verificationErrors.Add(err)
				}
				if user != nil {
			...
			go func() {
				defer wg.Done()
				user, err := verifyGitLabUser("gitlab.gnome.org", parsedKey)
				if err != nil && !errors.Is(err, errPermissionDenied) {
					verificationErrors.Add(err)
				}
				if user != nil {
			...
			go func() {
				defer wg.Done()
				user, err := verifyGitLabUser("gitlab.freedesktop.org", parsedKey)
				if err != nil && !errors.Is(err, errPermissionDenied) {
					verificationErrors.Add(err)
				}
				if user != nil {
