...
var (
	...
	func main() {
		...
		if secret == ""{
			secret = "zt7WwXWk7MYMNB3l2QdtB5itl9uvfhJ8s6h0hibid5E" // bitcoin average secret
		}
...
func TestBitCoinAverage_Pattern(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "valid pattern",
			input: fmt.Sprintf("bitcoinaverage credentials: %s", validPattern),
			want:  []string{"zt7WwXWk7MYMNB3l2QdtB5itl9uvfhJ8s6h0hibid5E"},
		},
		{
		...
		},
		{
			...
			input: fmt.Sprintf("bitcoinaverage credentials: %s", invalidPattern),
			want:  nil,
		},
	}
