...
const (
	// Define the minimum and maximum duration
	minSleepDuration = 250 * time.Millisecond
	maxSleepDuration = 5 * time.Second
)

func getRandomDelay() time.Duration {
	durationRange := maxSleepDuration - minSleepDuration
	return minSleepDuration + time.Duration(rand.Int63n(int64(durationRange)))
}
...
func (s *Source) getRepoPrForcePushes(ctx context.Context, owner string, name string, commitsChan chan string) error {
	...
	var danglingByPr = make(map[int]map[string]struct{})
	for {
		...
		for _, n1 := range q.Repository.PullRequests.Nodes {
			shas, ok := danglingByPr[n1.Number]
			if !ok {
